...
public class GenericUDFTumbledWindow extends GenericUDF {
  ...
  private final transient TimestampWritable timestampResult = new TimestampWritable();
  ...
  @Override public Object evaluate(DeferredObject[] arguments) throws HiveException {
    ...
    timestampResult.set(new Timestamp(truncate(ts, idt, origin)));
  ...
  private long truncate(Timestamp ts, HiveIntervalDayTime idt, Timestamp origin) {
    ...
    long offset = ts.getTime() % intervalDurationMs - origin.getTime() % intervalDurationMs;
    if (offset < 0) {
